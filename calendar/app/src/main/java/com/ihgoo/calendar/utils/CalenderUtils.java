package com.ihgoo.calendar.utils;
...
public class CalenderUtils {
     ...
     * @return
     */
    public CalenderUtils(Calendar cal) {
        mCurrenCalendar = cal;
        int leapMonth = 0;
        Date baseDate = null;
        try {
    ...
    public String toString() {
        ...
        if (StringUtils.isNullOrEmpty(message)) {
            String solarMsg = new SolarTermUtil(mCurrenCalendar)
                    .getSolartermsMsg();
            // 判断当前日期是否为节气
    ...
    public static String getCurrentDay(Calendar calendar) {
        return getDay(calendar) + " 农历" + new CalenderUtils(calendar).getDay()
                + " " + getWeek(calendar);
    }
